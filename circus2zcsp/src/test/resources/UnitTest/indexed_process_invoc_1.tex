...
\begin{circus}
    \circprocess\ P \circdef (i:\{~1,2,3~\} \circindex \circbegin \\
        \t1 \circstate\ State == [~ v: \nat | true ~] \\
        \t1 Init ~~==~~ [~ (State)' | v' = 0 ~] \\
        \t1 Read \circdef c!v \then \Skip \\
        \t1 Write \circdef d?x \then \Skip \\
        \t1 \circspot \lschexpract Init \rschexpract \circseq (\circmu X \circspot (Read \extchoice Write) \circseq X) \\ 
	\circend) \lcircindex 1 \rcircindex 
\end{circus}
%    \interleave
%    (i:\{~1,2,3~\} \circindex \circbegin \\
%        \t1 \circstate\ State == [~ v: \nat | true ~] \\
%        \t1 Init ~~==~~ [~ (State)' | v' = 0 ~] \\
%        \t1 Read \circdef c!v \then \Skip \\
%        \t1 Write \circdef d?x \then \Skip \\
%        \t1 \circspot \lschexpract Init \rschexpract \circseq (\circmu X \circspot (Read \extchoice Write) \circseq X) \\ 
%	\circend) \lcircindex 2 \rcircindex 
\end{circus}
